...
// 1. OrderEntryModal をファイルの先頭（インポートの下）に追加
...
// 2. Inline Modal を OrderEntryModal コンポーネント呼び出しに置換
const inlineModalStart = content.indexOf('<div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0');
const inlineModalEnd = content.indexOf('</div>\n    </div>\n  );', inlineModalStart) + 21;

if (inlineModalStart !== -1 && inlineModalEnd !== -1) {
  content = content.substring(0, inlineModalStart) + 
  `<OrderEntryModal 
    initialItem={editingItem} 
    onClose={() => setIsEntryModalOpen(false)} 
    onSuccess={() => { setIsEntryModalOpen(false); fetchItems(); }} 
  />` + content.substring(inlineModalEnd);
}

fs.writeFileSync(path, content, 'utf8');
console.log("Refactored inline modal to OrderEntryModal successfully.");
